...
void nhapsinhvien(sinhvien sv[])
{
	for (int i = 0; i < 3; i++)
	{
		...
		cin.getline(sv[i].sdt, 20);
		cout << "\t\t Nhap Diem Toan: ";
	do{	
	    cin >> sv[i].diemtoan;
		if(sv[i].diemtoan < 0 || sv[i].diemtoan > 10){
		...
		do{
		cin >> sv[i].diemly;
		if(sv[i].diemly<0 || sv[i].diemly > 10){
		...
		do{
		cin >> sv[i].diemRL;
		if(sv[i].diemRL<0 || sv[i].diemRL >10){
			cout<<"\t\tNhap lai diem ren luyen :"; 
		} 
		}
		while(sv[i].diemRL<0 || sv[i].diemRL >10); 
	}
}
...
void doctep(sinhvien sv[] , string c[])
{   
	fstream f;
